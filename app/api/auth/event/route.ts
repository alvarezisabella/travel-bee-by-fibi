...
export async function POST(req: NextRequest){
    ...
    const {itineraryid, title, description, status, startTime, duration, day, location, type} = await req.json()
    ...
    const {data, error} = await insertEvent(supabase,{
        itinerary_id: itineraryid, title, description, status, starts_at: startTime || undefined, ends_at, day: day || undefined, location, type, created_by: user.id,})
...
export async function PUT(req: NextRequest) {
    const {id, title, description, status, startTime, duration, location, type} = await req.json()
    ...
    const {error} = await updateEvent(supabase, id, { title, description, status, starts_at: startTime || undefined, ends_at, location, type })
    if(error) { return NextResponse.json({ error: error.message }, { status: 500 }) }
