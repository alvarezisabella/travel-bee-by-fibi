...
export async function insertEvent(supabase: SupabaseClient,
  data: { itinerary_id: string; title: string; description?: string
    ...
    type?: string; status?: string; travelers?: string[]; created_by: string; lat?:number; lng?:number, updated?:string, updated_by?:string}
) {
...
export async function getEvent(supabase: SupabaseClient, id: string) {
  return supabase
    .from('events')
    .select('id, itinerary_id, title, description, starts_at, ends_at, num_guests, location, cost, booking_code, type, status, created_by, created_at, updated_at, lat, lng, updated, updated_by')
    .eq('id', id)
    .single()
}
...
export async function getEventsByItinerary(supabase: SupabaseClient, itineraryId: string) {
  return supabase
    .from('events')
    .select('id, itinerary_id, title, description, starts_at, ends_at, location, cost, booking_code, type, status, day, upvote, downvote, guests_count, travelers, created_by, created_at, updated_at, lat, lng, updated, updated_by')
...
export async function updateEvent(
  ...
  data: { title?: string; description?: string; starts_at?: string; ends_at?: string
    ...
    booking_code?: string; type?: string; status?: string; travelers?: string[]; lat?:number; lng?:number; updated_by?:string;
  }
) {
